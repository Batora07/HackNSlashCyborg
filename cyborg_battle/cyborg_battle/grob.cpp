	...
	if (state == GROB_STATE_IDLE || state == GROB_STATE_MOVE) {
		...
		if (thinkTimer <= 0) {
			...
			if (action < 3) {
				...
				angle = rand() % 360; // randome angle to move in
				moving = true;
				changeAnimation(GROB_STATE_MOVE, state != GROB_STATE_MOVE);
			}
			else {
				// otherwise juste be happy and idle :)
				moving = false;
				changeAnimation(GROB_STATE_IDLE, true);
			}
		}
...
void Grob::changeAnimation(int newState, bool resetFrameToBeginning) {
	...
	if (resetFrameToBeginning)
		...
		currentFrame = currentAnim->getFrame(currentFrame->frameNumber); // change direction for example, wanna change animation but not the frame we were on
}
...
void Grob::updateAnimation() {
	...
	//if should show running animation, let's change the state properly
	if (state != GROB_STATE_MOVE && moving) 
	...
	if (frameTimer >= currentFrame->duration) {
		...
		if (currentFrame->frameNumber == currentAnim->getEndFrameNumber()) {
			//depends on current animation and what's going on a bit
