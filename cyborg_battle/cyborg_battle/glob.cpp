	...
	if (state == GLOB_STATE_IDLE || state == GLOB_STATE_MOVE) {
		...
		if (thinkTimer <= 0) {
			...
			if (action < 3) {
				...
				findNearestTarget();
				// found a target alive, lets go hunting
				if (target != NULL && target->hp > 0) {
					float dist = Entity::distanceBetweenTwoEntities(this, target);
					// if in range attack
					if (dist < 100) {
						telegraph(); // telegraph our attack first so player has a chance to dodge
						aiState = GLOB_AI_NORMAL;
					}
					else {
...
void Glob::updateAnimation() {
	...
	// if we re in movestate but not actually going anywhere then just go back to idle
	...
	if (frameTimer >= currentFrame->duration) {
		...
		if (currentFrame->frameNumber == currentAnim->getEndFrameNumber()) {
			if (state == GLOB_STATE_TELEGRAPH) {
				...
				frameTimer = 0;
				// if some how alive then change state back to miving if hp>0
				if (hp > 0) {
					changeAnimation(GLOB_STATE_MOVE, true);
				}
				else {
